...
Writes:
    results/fig1_raw_population.png
    results/fig2_normalized_sensitivity.png
    results/fig3_sobol_s1_st.png
    results/fig4_sobol_st_comparison.png
...
# ---------------------------------------------------------------------------
# Figure 1 — Raw final population vs F10.7 (log scale)
# ---------------------------------------------------------------------------
...
ax.set_title("Figure 1.  Final Debris Population vs Solar Activity")
ax.set_yscale("log")
ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:,.0f}"))
ax.legend()
ax.grid(alpha=0.25, which="both")
fig.tight_layout()
save(fig, "fig1_raw_population.png")

# ---------------------------------------------------------------------------
# Figure 2 — Normalized population (N / N at F10.7 = 70)
# ---------------------------------------------------------------------------
...
ax.axvline(150, color="gray", ls=":", lw=1.2, alpha=0.7, label="Baseline (F10.7 = 150)")
ax.set_xlabel("F10.7 Solar Flux [sfu]")
ax.set_ylabel("Normalized Population  (N / N\u2070)")
ax.set_title("Figure 2.  Normalized Sensitivity to Solar Activity")
ax.legend()
ax.grid(alpha=0.25)
fig.tight_layout()
save(fig, "fig2_normalized_sensitivity.png")

# ---------------------------------------------------------------------------
# Figure 3 — S1 and ST grouped bar chart (all parameters, both models)
# ---------------------------------------------------------------------------
...
ax.set_title("Figure 3.  First-Order (S1) and Total-Order (ST) Sobol Sensitivity Indices")
ax.axhline(0, color="black", lw=0.8)
ax.legend(fontsize=9, ncol=2)
ax.grid(axis="y", alpha=0.25)
fig.tight_layout()
save(fig, "fig3_sobol_s1_st.png")

# ---------------------------------------------------------------------------
# Figure 4 — Total-order ST side-by-side across models
# ---------------------------------------------------------------------------
fig, ax = plt.subplots(figsize=(7, 5))
...
ax.set_title("Figure 4.  Parameter Importance Comparison (Total-Order ST)")
ax.axhline(0, color="black", lw=0.8)
ax.legend()
ax.grid(axis="y", alpha=0.25)
fig.tight_layout()
save(fig, "fig4_sobol_st_comparison.png")
